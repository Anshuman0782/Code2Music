...
//   // Match identifiers, numbers, strings, operators, symbols, etc.
//   const regex =
//     /[A-Za-z_]\w*|[{}()[\];,.]|==|!=|<=|>=|\+\+|--|&&|\|\||[=+\-*/<>]|".*?"|'.*?'|\d+/g;
...
//   // Keywords (C, C++, Java, JS, Python, PHP)
//   if (
//     /^(if|else|for|while|do|switch|case|break|continue|return|try|catch|finally|throw|new|class|struct|interface|extends|implements|import|package|public|private|protected|static|void|int|float|double|char|long|short|boolean|bool|true|false|null|this|super|def|lambda|async|await|echo|print|var|let|const|function)$/.test(
//       token
//     )
//   )
//     return "keyword";
...
//   // HTML tags
//   if (/^<\/*[a-zA-Z]+[0-9]*>$/.test(token)) return "html-tag";

//   // CSS properties
//   if (
//     /^(color|background|margin|padding|border|width|height|font|display|position|flex|grid)$/.test(
//       token
//     )
//   )
//     return "css-property";
...
//   // Default = identifier (variable names, etc.)
//   return "identifier";
// }
...
//     // --- Check for missing semicolons (C/Java/JS/PHP only)
//     if (["c", "cpp", "java", "javascript", "php"].includes(language)) {
//       const trimmed = line.trim();
//       if (
//         trimmed &&
//         !/[;{}:]$/.test(trimmed) &&
//         !/^(if|else|for|while|switch|function)/.test(trimmed)
//       ) {
...
//         trimmed &&
//         !/[;{}:]$/.test(trimmed) &&
//         !/^(if|else|for|while|switch|function|class|try|catch)/.test(trimmed)
//       ) {
//         errors.push({ line: lineNo, message: "Possible missing semicolon" });
//       }
//     }
